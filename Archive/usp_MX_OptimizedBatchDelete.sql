...
DECLARE @RetainDate VARCHAR(10)              -- 保留日期（此日期之前的記錄將被刪除）
...
BEGIN TRY
    ...
    BEGIN
        ...
        SET @TotalBatchDuration = @TotalBatchDuration + @BatchDuration
        SET @BatchCount = @BatchCount + 1
        
        -- 動態調整批次大小
        IF @BatchDuration < 1000 AND @BatchSize < @MaxBatchSize
            SET @BatchSize = @BatchSize * 2  -- 如果執行太快，增加批次大小
        ELSE IF @BatchDuration > 5000 AND @BatchSize > @MinBatchSize
            SET @BatchSize = @BatchSize / 2  -- 如果執行太慢，減少批次大小
